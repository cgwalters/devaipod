...
# Install bubblewrap (required for sandboxing)
install_bubblewrap() {
    local pkg_manager="$1"
    ...
    case "$pkg_manager" in
        apt)
            install_packages apt bubblewrap
            ;;
        apk)
            install_packages apk bubblewrap
            ;;
        dnf|yum)
            install_packages "$pkg_manager" bubblewrap
            ;;
        *)
            echo "WARNING: Cannot install bubblewrap, sandbox may not work"
            return 1
            ;;
    esac
}
...
main() {
    ...
    install_bubblewrap "$pkg_manager" || true
