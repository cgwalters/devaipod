...
Devaipod supports OpenCode, Goose, and Claude Code via ACP. Configure agent profiles in `~/.config/devaipod.toml`:
...
- A single string: `default = "opencode"`
- An ordered array: `default = ["claude", "goose", "opencode"]`

When multiple profiles are listed, devaipod probes the agent container for each binary in order and selects the first one found. Multi-agent devcontainer images work without per-project configuration -- devaipod detects which agents are installed.

If `default` is unset or all probes fail, devaipod falls back to `["opencode", "acp"]`.

### Profile Definition

Each profile under `[agent.profiles.<name>]` defines:

- `command`: Array of strings, the command to execute (e.g. `["goose", "acp"]`)
- `env`: Optional environment variables (e.g. model selection, permissions)
...
The agent container image must include:

- The agent binary (opencode, goose, claude-agent-acp)
- Git and your development tools
- Agent config files if not using defaults

See [ACP Support](acp.md) for example devcontainer images.
