...
pub fn generate_compose(
    ...
    let compose = ComposeConfig {
        name: Some(format!("devaipod-{project_name}")),
        services,
        volumes,
    };

    // Generate the augmented devcontainer.json
    let augmented_devcontainer =
        generate_augmented_devcontainer(devcontainer, project_name, workspace_folder)?;
...
# Future: Add devaipod utilities here if needed
"#
    )
}
